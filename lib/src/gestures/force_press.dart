// -----------------------------------------------------------------------------
// Forked from Flutter's original 'src/gestures/force_press.dart' file.
// -----------------------------------------------------------------------------
...
enum _ForceState {
  ...
  // the DetailedForcePressGestureRecognizer is the only recognizer in the arena, thus
  ...
  // has just become greater than the DetailedForcePressGestureRecognizer.startPressure.
  started,

  // A pointer is down and the pressure of the pointer has just become greater
  // than the DetailedForcePressGestureRecognizer.peakPressure. Even after a pointer
  // crosses this threshold, onUpdate callbacks will still be sent.
  peaked,
}

/// Signature used by a [DetailedForcePressGestureRecognizer] for when a pointer has
/// pressed with at least [DetailedForcePressGestureRecognizer.startPressure].
typedef DetailedGestureForcePressStartCallback = void Function(ForcePressDetails details, PointerEvent event);

/// Signature used by [DetailedForcePressGestureRecognizer] for when a pointer that has
/// pressed with at least [DetailedForcePressGestureRecognizer.peakPressure].
typedef DetailedGestureForcePressPeakCallback = void Function(ForcePressDetails details, PointerEvent event);

/// Signature used by [DetailedForcePressGestureRecognizer] during the frames
/// after the triggering of a [DetailedForcePressGestureRecognizer.onStart] callback.
typedef DetailedGestureForcePressUpdateCallback = void Function(ForcePressDetails details, PointerEvent event);

/// Signature for when the pointer that previously triggered a
/// [DetailedForcePressGestureRecognizer.onStart] callback is no longer in contact
/// with the screen.
typedef DetailedGestureForcePressEndCallback = void Function(ForcePressDetails details, PointerEvent event);
...
/// [DetailedForcePressGestureRecognizer.startPressure]. A long press recognizer will
/// win when the press down time exceeds the threshold time as long as the
/// pointer's pressure was never greater than
/// [DetailedForcePressGestureRecognizer.startPressure] in that duration.
///
...
/// custom curve.
class DetailedForcePressGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  DetailedForcePressGestureRecognizer({
  ...
  DetailedGestureForcePressStartCallback? onStart;
  ...
  DetailedGestureForcePressUpdateCallback? onUpdate;
  ...
  /// The position of the pointer is provided in the callback's `details`
  /// argument, which is a [ForcePressDetails] object.
  DetailedGestureForcePressPeakCallback? onPeak;

  /// A pointer is no longer in contact with the screen.
  ///
  /// The position of the pointer is provided in the callback's `details`
  /// argument, which is a [ForcePressDetails] object.
  DetailedGestureForcePressEndCallback? onEnd;
